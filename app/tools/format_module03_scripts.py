# -*- coding: utf-8 -*-
"""
Régénère les scripts Word du Module 3 (attentes comportementales) à partir de :
- tools/module03_rich_content.py (Leçons 27–32)

Même gabarit que modules 1 et 2 : cadre → الهدف → Séquences.

**Sortie par défaut : V2 seulement** — écrit ``…-V2.docx`` ; le .docx **canonique** (sans ``-V2``) **n’est pas modifié**.
Texte en style normal dans le V2 (pas de surlignage de diff vs V1).

Usage (depuis la racine du dépôt) :
  python app/tools/format_module03_scripts.py
  python app/tools/format_module03_scripts.py --lesson 27
  python app/tools/format_module03_scripts.py --in-place   # rare : écrase le V1 (éviter en production)
"""
...
from module03_rich_content import LESSONS as LESSONS_M3
...
def _out_path(base: Path, *, v2: bool) -> Path:
    """V2 : ``…-V2.docx`` à côté du canonique (jamais ``…-V2-V2.docx``). V1 : retourne ``base``."""
    if not v2:
        return base
    stem = base.stem
    if stem.endswith("-V2"):
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Régénère les scripts Word module 3 (Python → .docx).")
    ap.add_argument(
        "--in-place",
        action="store_true",
        help="Écrase le .docx canonique (V1). Par défaut : écrit uniquement -V2.docx, V1 inchangé.",
    )
    ap.add_argument(
        "--lesson",
        type=int,
        default=0,
        metavar="N",
        help="Ne régénérer que la leçon N (27–32). 0 = toutes.",
    )
    args = ap.parse_args()
    v2 = not args.in_place
    lesson_filter: int | None = None if args.lesson == 0 else args.lesson
    _write_module("03-Module*", LESSONS_M3, lesson_id=lesson_filter, v2=v2)
    msg = "Terminé module 3"
    if v2:

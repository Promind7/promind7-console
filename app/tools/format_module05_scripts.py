# -*- coding: utf-8 -*-
"""
Régénère les scripts Word du Module 5 (trouver un emploi) à partir de :
- tools/module05_rich_content.py (Leçons 44–51)

Même gabarit que Module 3–4 : cadre → الهدف → Séquences ; **…** → gras Word.
...
Usage :
  python app/tools/format_module05_scripts.py
  python app/tools/format_module05_scripts.py --lesson 44
  python app/tools/format_module05_scripts.py --in-place
"""
...
from module05_rich_content import LESSONS as LESSONS_M5
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Régénère les scripts Word module 5 (Python → .docx).")
    ap.add_argument(
        "--in-place",
        action="store_true",
    ...
    ap.add_argument(
        ...
        default=0,
        metavar="N",
        help="Ne régénérer que la leçon N (44–51). 0 = toutes.",
    )
    args = ap.parse_args()
    v2 = not args.in_place
    lesson_filter: int | None = None if args.lesson == 0 else args.lesson
    _write_module("05-Module*", LESSONS_M5, lesson_id=lesson_filter, v2=v2)
    msg = "Terminé module 5"
    if v2:

# -*- coding: utf-8 -*-
"""
Régénère les scripts Word du Module 4 (trouver un stage) à partir de :
- tools/module04_rich_content.py (Leçons 33–43)

Même gabarit que Module 3 : cadre → الهدف → Séquences ; **…** → gras Word.
...
Usage :
  python app/tools/format_module04_scripts.py
  python app/tools/format_module04_scripts.py --lesson 33
  python app/tools/format_module04_scripts.py --in-place   # exception : écrase le V1
"""
...
from module04_rich_content import LESSONS as LESSONS_M4
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Régénère les scripts Word module 4 (Python → .docx).")
    ap.add_argument(
        "--in-place",
        action="store_true",
    ...
    ap.add_argument(
        ...
        default=0,
        metavar="N",
        help="Ne régénérer que la leçon N (33–43). 0 = toutes.",
    )
    args = ap.parse_args()
    v2 = not args.in_place
    lesson_filter: int | None = None if args.lesson == 0 else args.lesson
    _write_module("04-Module*", LESSONS_M4, lesson_id=lesson_filter, v2=v2)
    msg = "Terminé module 4"
    if v2:

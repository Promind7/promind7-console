# -*- coding: utf-8 -*-
"""
Régénère les scripts Word du Module 7 (langues pro) à partir de :
- tools/module07_rich_content.py (Leçons 57–63)

Même gabarit que Module 3–6 : cadre → الهدف → Séquences ; **…** → gras Word.
...
Usage :
  python app/tools/format_module07_scripts.py
  python app/tools/format_module07_scripts.py --lesson 57
  python app/tools/format_module07_scripts.py --in-place
"""
...
from module07_rich_content import LESSONS as LESSONS_M7
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Régénère les scripts Word module 7 (Python → .docx).")
    ap.add_argument(
        "--in-place",
        action="store_true",
    ...
    ap.add_argument(
        ...
        default=0,
        metavar="N",
        help="Ne régénérer que la leçon N (57–63). 0 = toutes.",
    )
    args = ap.parse_args()
    v2 = not args.in_place
    lesson_filter: int | None = None if args.lesson == 0 else args.lesson
    _write_module("07-Module*", LESSONS_M7, lesson_id=lesson_filter, v2=v2)
    msg = "Terminé module 7"
    if v2:
